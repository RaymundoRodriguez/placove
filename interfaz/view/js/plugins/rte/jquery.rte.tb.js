...
if (navegador == "Microsoft Internet Explorer"){
    ...
    font:{command:"fontname",select:'<select>	<option value="">- font -</option>	<option value="arial">Arial</option>	<option value="comic sans ms">Comic Sans</option>	<option value="courier new">Courier New</options>	<option value="georgia">Georgia</option>	<option value="helvetica">Helvetica</options>	<option value="impact">Impact</option>	<option value="times new roman">Times</options>	<option value="trebuchet ms">Trebuchet</options>	<option value="verdana">Verdana</options></select>	',tags:["font"]},
    size:{command:"fontsize",select:'<select>	<option value="">-</option>	<option value="1">1 (8pt)</option>	<option value="2">2 (10pt)</option>	<option value="3">3 (12pt)</options>	<option value="4">4 (14pt)</option>	<option value="5">5 (16pt)</options>	<option value="6">6 (18pt)</option>	<option value="7">7 (20pt)</options></select>	',tags:["font"]},
    ...
    justifyRight:{command:"justifyright"},
    justifyFull:{command:"justifyfull"},
    s3:{separator:true},indent:{command:"indent"},
    outdent:{command:"outdent"},s4:{separator:true},
    subscript:{command:"subscript",tags:["sub"]},
    superscript:{command:"superscript",tags:["sup"]},
    s5:{separator:true},
    orderedList:{command:"insertorderedlist",tags:["ol"]},
...
function lwrte_block_compare(b,a){a=a.replace(/<([^>]*)>/,"$1");return(a.toLowerCase()==b.nodeName.toLowerCase())}
function lwrte_style_init(b){var a=this;a.select='<select><option value="">- no css -</option></select>';
    if(b.css.length){
        $.ajax({
            url:"styles.php",
            type:"POST",
            data:{css:b.css[b.css.length-1]},
            async:false,
            success:function(f){
                var e=f.split(",");
                var c="";for(var d in e){
...
function lwrte_style(b){
    ...
    function lwrte_color(){
        var p=this;
        var a=p.create_panel("Seleccionar Color Para El Texto",385);
        var n=false;
        var j=false;
        ...
        for(var h=0;h<b.length;h++){$("<div></div>").addClass("item").css("background",b[h]).appendTo(d)}
        var o=$("#rgb").height();var k=$("#rgb").width()/6;$("#rgb,#gray,#palette",a).mousedown(function(i){
            n=true;
            return false}).mouseup(function(i){
            n=false;
            return false}).mouseout(function(i){
            j=false;
            return false}).mouseover(function(i){
            j=true;
            return false});
        ...
        $("#ok",a).click(function(){
            ...
            return false});
        function g(q){
            var i="0123456789abcdef";return i.charAt(Math.floor(q/16))+i.charAt(q%16)
        }
        function e(q){
            var s={x:q.offsetLeft,y:q.offsetTop};if(q.offsetParent){var i=e(q.offsetParent);s.x+=i.x;s.y+=i.y}
            return s
        }
        function c(s,r){
            var i,u;r=r||window.event;
            var q=r.target||r.srcElement;
            var t=e(s);i=r.pageX-t.x;u=r.pageY-t.y;
            return{x:i,y:u}}
        function m(u,q,E,s,z){
            ...
            var B=w.y;
            if(q){
                i=(D>=0)*(D<k)*255+(D>=k)*(D<2*k)*(2*255-D*255/k)+(D>=4*k)*(D<5*k)*(-4*255+D*255/k)+(D>=5*k)*(D<6*k)*255;v=(D>=0)*(D<k)*(D*255/k)+(D>=k)*(D<3*k)*255+(D>=3*k)*(D<4*k)*(4*255-D*255/k);C=(D>=2*k)*(D<3*k)*(-2*255+D*255/k)+(D>=3*k)*(D<5*k)*255+(D>=5*k)*(D<6*k)*(6*255-D*255/k);
                var t=(o-B)/o;i=128+(i-128)*t;v=128+(v-128)*t;C=128+(C-128)*t
            }
            else{
                if(E){
                    i=v=C=(o-B)*1.7
                    ...
                    if(s){
                            D=Math.floor(D/10);B=Math.floor(B/10);A=b[D+B*5]
                        }
                    }
                }
                if(!s){
