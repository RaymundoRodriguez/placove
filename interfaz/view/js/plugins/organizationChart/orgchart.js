...
(function($) {
  ...
  function buildNode($node, $appendTo, level, opts) {
    ...
    $nodeDiv = $("<div>").addClass("node").append($heading).attr("id", $node.attr("id"));
    $nodeCell.append($nodeDiv);
    $nodeRow.append($nodeCell);
    $tbody.append($nodeRow);
    
	$nodeDiv.click(function(){
		var id_complet=$(this).attr("id"),
		id_jerarquia=id_complet.substring(3);
		getMiniTree(id_jerarquia);
		getPosition(id_jerarquia);
	});
